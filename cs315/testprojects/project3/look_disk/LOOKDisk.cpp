...
DiskDoneEvent * LOOKDisk::proccessRequest ( RequestEvent * reqEvent, int currentT ) {

    int width = 10;
    if (write == 0) {

        std::ofstream lookOutput;
        lookOutput.open("./LOOK_output.txt");

        lookOutput << "LOOK REPORT" << std::endl;
        lookOutput << std::left
                   << std::setw(width) << '#'
                   << std::setw(width) << "Trac"
                   << std::setw(width) << "Sec"
...
DiskDoneEvent * LOOKDisk::proccessDiskDone ( DiskDoneEvent * diskDoneE, int currentT ) {
    ...
    if (!waitQueue->isEmpty()) { //same part as process request event
        ...
        std::ofstream lookOutput;
        lookOutput.open("./LOOK_output.txt", std::ios_base::app);
...
void LOOKDisk::printDisk() {
    ...
    if (waitQueue->isEmpty()) {
        std::cout << "\tLOOK UP Queue: EMPTY" << std::endl << std::endl;
    }
    else {
        std::cout << "\tLOOK UP Queue:" << std::endl;
        waitQueue->print();
        std::cout << std::endl;
    }
}

void LOOKDisk::printSummary() {

  int width = 10;
  std::ofstream fcfsOutput;
  fcfsOutput.open("./LOOK_summary.txt");
  ...
  fcfsOutput << std::endl << "LOOK " << std::fixed << std::setprecision(2) << std::setw(width) << minTS
