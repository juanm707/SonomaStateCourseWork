...
DiskDoneEvent * FCFSDisk::proccessRequest ( RequestEvent * reqEvent, int currentT ) {

    int width = 10;
    if (write == 0) {

        std::ofstream fcfsOutput;
        fcfsOutput.open("./FCFS_output.txt");

        fcfsOutput << "FCFS REPORT" << std::endl;
        fcfsOutput << std::left
                  << std::setw(width) << '#'
                  << std::setw(width) << "Trac"
                  << std::setw(width) << "Sec"
    ...
    if (state == 0) {
        ...
        Request * getReq_1 = waitQueue->getRequest();
        numRequests--;
        currentDiskTime = getReq_1->time();

        /*------------------------------------------------------------------*/

        std::ofstream fcfsOutput;
        fcfsOutput.open("./FCFS_output.txt",std::ios_base::app);

        int track_v2 = getReq_1->track();
        int sector_v2 = getReq_1->sector();

        double entr_v2 = getReq_1->time() * 0.1;
        ...
        fcfsOutput << std::left << std::setw(width)
                  << count + 1
        ...
        double seekTime = abs(_track - getReq_1->track()) * 3;
        double rotationalD = 0;

        if (_sector < getReq_1->sector()) {

            rotationalD = abs(_sector - getReq_1->sector()) * 1;

        } else if (_sector >= getReq_1->sector()) {

            rotationalD = ((30 - _sector) + getReq_1->sector()) * 1;
        }

        float transferTime = 1;
        double serve = seekTime + rotationalD + transferTime;

        DiskDoneEvent *servEvent_1 = new DiskDoneEvent( serve + currentT, type);
        state = 1;

        //update rw head
        _track = getReq_1->track();
        _sector = getReq_1->sector() + 1;
...
DiskDoneEvent * FCFSDisk::proccessDiskDone ( DiskDoneEvent * diskDoneE, int currentT ) {
    ...
    if (!waitQueue->isEmpty() ) { //same part as process request event
        ...
        Request * getReq_3 = waitQueue->getRequest();
        numRequests--;
        currentDiskTime = getReq_3->time();

        /*-------------------------------------------------------------------------*/
        std::ofstream fcfsOutput;
        fcfsOutput.open("./FCFS_output.txt", std::ios_base::app);

        int track_v2 = getReq_3->track();
        int sector_v2 = getReq_3->sector();

        double entr_v2 = getReq_3->time() * 0.1;
        ...
        // calculate serivce time
        double seekTime = abs(_track - getReq_3->track()) * 3;
        double rotationalD = 0;

        if (_sector < getReq_3->sector()) {

            rotationalD = abs(_sector - getReq_3->sector()) * 1;

        } else if (_sector > getReq_3->sector()) {

            rotationalD = ((30 - _sector) + getReq_3->sector()) * 1;
        }
        ...
        //update rw head
        _track = getReq_3->track();
        _sector = getReq_3->sector() + 1;
...
void FCFSDisk::printDisk() {
    ...
    if (server == nullptr) {
        ...
        std::cout << "\tServer: (serv " << server->time() << ' ' << server->track() << ' ' << server->sector() << ')' << std::endl << std::endl;
    }

    if (waitQueue->isEmpty()) {
        std::cout << "\tFCFS Queue: EMPTY" << std::endl << std::endl;
    }
...
void FCFSDisk::printSummary() {

    int width = 10;
    std::ofstream fcfsOutput;
    fcfsOutput.open("./FCFS_summary.txt");
