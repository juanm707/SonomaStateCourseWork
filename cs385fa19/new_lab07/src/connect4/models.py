...
class C4Session:
    ...
    def check_winner(self, idx):

        player_str = self.board[idx]
        if player_str not in ['1', '2']:
            return False
        ...
        deltas = [
            (1, 1, 'NE'),   # north east direction
            (-1, 1, 'SE'),  # south east direction
            (1, 0, 'N'),   # north
            (0, 1, 'E'),   # eash
        ]
        
        for row_inc, col_inc, direction in deltas:
