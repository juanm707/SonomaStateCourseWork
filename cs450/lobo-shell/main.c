...
int main() {
    ...
    while (fgets(line, MAX_LINE_CHARS, stdin)) {
        ...
        for (int i = 0; i < num_words; i++) {
            ...
            int dontWait = 0; // example: [ cat < doesntExist.txt ] cat will wait but shouldnt

            if (num_words > 1) { // If we at least have some cmd and something else like <,>,|,file.*,-x etc...
                errnum = 0;
                char * buildString = NULL; // incase there are quotes
                ...
                for (int j = i + 1; j < num_words + 1; j++) { // Looping through the WHOLE line
                    ...
                    if (line_words[j] == NULL || *(line_words[j]) == PIPE || *(line_words[j]) == INPUT_REDIRECT ||
                        *(line_words[j]) == OUTPUT_REDIRECT || line_words[j] == OUTPUT_APPEND) {
                        ...
                        if (line_words[j] == NULL) {
                            ...
                            if (outRedir_or_pipe == NULL) {
                            ...
                            else if (*outRedir_or_pipe == OUTPUT_REDIRECT || out_redirection == OUTPUT_APPEND) {
                                char * outputFileName = line_words[++j];
                                //printf("Redirect Output File Name: %s\n", outputFileName);
                                if (strcmp(line_words[o], OUTPUT_APPEND) == 0) {
                                    out_fd = open(outputFileName, O_WRONLY | O_CREAT | O_APPEND, 0664); // 0664 = user: r-w, group: r-w, other: r
                                }
                                else if (*(line_words[o]) == OUTPUT_REDIRECT) {
                                ...
                                if (errnum && errnum != 10) { // 10 because of a 'no child processes', idk why im getting it
                                    //fprintf(stderr, "Value of errno: %d\n", errno); // error checking
                                    //perror("Error printed by perror"); // error checking
                                    fprintf(stderr, "-lobo-shell-198: %s: %s\n", outputFileName, strerror(errnum)); // error checking
                                    dontWait = 1;
                                }
                            ...
                            if (errnum && errnum != 10) { // 10 because of a 'no child processes', idk why im getting it
                                //fprintf(stderr, "Value of errno: %d\n", errno); // error checking
                                //perror("Error printed by perror"); // error checking
                                fprintf(stderr, "-lobo-shell: %s: %s\n", inputFilename, strerror(errnum)); // error checking
                                dontWait = 1;
                            }

                            in_redirection = 1;
                        }
                        else if (*(line_words[j]) == OUTPUT_REDIRECT || line_words[j] == OUTPUT_APPEND) {
                            int o = j;
                            char * outputFilename = line_words[++j];
                            // check for the next possible thing: pipe or output redirection, where is output going
                            char * inputRedir_or_pipe = line_words[++j];
                            if (inputRedir_or_pipe == NULL) {
                                //do nothing
                            }
                            else if (*inputRedir_or_pipe == PIPE) {
                                // We found a pipe
                                // Two pipes will be "reused"
                                if (_pipe) {
                                    // if pipe was already set, we need to create a second pipe
                            ...
                            else if (*inputRedir_or_pipe == INPUT_REDIRECT) {
                                char * inputFileName = line_words[++j];
                                //printf("Redirect Input File Name: %s\n", inputFileName);
                            ...
                            char * oapp = line_words[o];
                            if (strcmp(oapp, OUTPUT_APPEND) == 0) {
                                out_fd = open(outputFilename, O_WRONLY | O_CREAT | O_APPEND, 0664); // 0664 = user: r-w, group: r-w, other: r
                            }
                            else if (*(line_words[o]) == OUTPUT_REDIRECT) {
                                out_fd = open(outputFilename, O_WRONLY | O_CREAT | O_TRUNC, 0664); // 0664 = user: r-w, group: r-w, other: r
                            ...
                            if (errnum && errnum != 10) { // 10 because of a 'no child processes', idk why im getting it
                                //fprintf(stderr, "Value of errno: %d\n", errno); // error checking
                                //perror("Error printed by perror"); // error checking
                                fprintf(stderr, "-lobo-shell-301: %s: %s\n", outputFilename, strerror(errnum)); // error checking
                                dontWait = 1;
                            }
                            out_redirection = 1;
                        }
                        ...
                        switch (pid = fork()) {
                            ...
                            case 0:
                                ...
                                if (in_redirection && !dontWait) {
                                    ...
                                    close(fd);
                                } else if (out_redirection && !dontWait) {
                                    dup2(out_fd, 1);
                                    if (in_redirection) {
                                        dup2(fd, 0);
                                        close(fd);
                            ...
                            default:
                                ...
                                errno = 0; // reset to 0 because we might have already closed a fd and errno would be -1
                                break;
                        }
                    } else {
                        // build argument array
                        if (*line_words[j] == '"' || *line_words[j] == '\'') {

                            buildString = malloc(10000); // A different number? Actual size?
                            parsingQuote = 1;
                            int doneParsingQuote = 0;

                            int a = 1;
                            int b = 0;

                            char * thingInQuotes = line_words[j]; // first word \"????? ...(\" or ??")
                            char c = thingInQuotes[a];

                            while (c != '"' && c != '\'') {
                                while (!doneParsingQuote) {
                                    while (c != '\0' && !doneParsingQuote) {
                                        buildString[b] = c;
                                        c = thingInQuotes[++a];
                                        ++b;
                                        if (c == '"' || c == '\'')
