...
int main()
{
  ...
  // Prob of each class
  float class_S = 0.005;
  float class_I = 1 - class_S;
  cout << "<Using standard prior probablilities p(Spam) = " << class_S << ", p(Ham) = " << class_I << '>' << endl; 
  ...
  cout << "False positive rate = " << (fp/ignore2) * 100 << '%' << endl;
  cout << "False negative rate = " << (fn/ignore1) * 100 << '%' << endl;
  cout << fp << ' ' << ignore2 << endl;
  return 0;
}
